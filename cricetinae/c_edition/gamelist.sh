...
#rom清单来源https://www.oldmanemu.net/，种类有3ds、nds、gba、gbc、gb、psp、ps3、ps2、ps1、dc、n64、sfc、md、fc
...
do
	...
	sed '{1,/[^文][完游][整戏]清单/d;/下载地址/,$d;s/<[^<>]*>//g;s/&#8211;/-/g;s/&nbsp;/ /g;s/&amp;/\&/g;/^ *$/d}' $tmp_file|sed -n 'w '"$tar_list"''
	rm $tmp_file
done
